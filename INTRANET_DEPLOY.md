# 经济评审管理平台 · 内网（无互联网出口）离线部署方案

> 目标：把现有系统完整搬到贵单位**内网服务器**（无互联网出口、仅内网可访问），做到**断网也能完整运行**。
> 本文为独立部署文档，配套一键脚本位于 `deploy/setup-intranet.sh`。
>
> 关联文档：根目录 [README.md](README.md)（功能与架构）、[PRODUCTION_DEPLOY.md](PRODUCTION_DEPLOY.md)（公网 / MySQL 部署）。

---

## 一、离线能不能跑？——阻塞点分析

原系统在生产（阿里云）运行良好，但有几处**强依赖互联网**，内网断网会直接白屏 / 功能失效。本轮改造已逐项消除：

| 阻塞点 | 原实现 | 内网后果 | 本方案解决 |
|---|---|---|---|
| **前端 CDN** | `index.html` 引了 jsDelivr 的 Bootstrap / Bootstrap Icons / Chart.js | 断网后样式与图表全崩、页面白屏 | ✅ 已把 4 个资源**下载到 `frontend/vendor/`**，并改为本地 `/vendor/...` 引用（仓库内 `grep` 已确认 0 处 `cdn.jsdelivr` 残留） |
| **Node 依赖** | `npm install` 需访问 npm 源 | 断网无法安装依赖 | ✅ 已打包 `backend/deps/node_modules.tar.gz`（纯 JS、无原生二进制、跨 Linux 可移植），脚本自动解压，**无需 npm** |
| **数据库** | 生产用 MySQL | 内网若无 MySQL 则起不来 | ✅ 系统**默认 `json` 文件驱动**（零外部数据库），一条命令即可跑；MySQL 改为可选项 |
| **JWT 密钥** | 缺省随机 | 重启登录态全失效 | ✅ 脚本生成 `.env` 写入随机 `JWT_SECRET`，稳定且各实例独立 |
| **字体 / 其他外链** | 仅系统字体，无 Google Fonts | 无影响 | ✅ 已复核，除上面 4 个 CDN 外无其它外链 |

**结论**：改造后整个仓库自包含，连 `node_modules.tar.gz` 与 `vendor/` 一起拷到内网服务器即可离线运行。
...
2. **Node.js ≥ 18**：内网无法 `apt/yum install`，请**提前在有网的机器下载 Node 二进制**（官网 `node-vXX-linux-x64.tar.xz`），拷到内网后解压并把 `bin/node` 放入 `PATH`，或部署前 `export NODE_BIN=/path/to/node`。验证：`node -v` 应显示 v18+。
3. **（可选）MySQL**：若希望用 MySQL 而非默认 json 驱动，需内网已有 MySQL 8（同样需离线预装 / 由单位提供）并建库建用户。详见第七节。
...
`SEED_DEFAULT_USERS=true` 改为 `false` 后重启（`./setup-intranet.sh restart`，或
`SEED_DEFAULT_USERS=false ./setup-intranet.sh restart`）。

---

## 五、网络隔离：只允许单位内网访问

系统监听 `0.0.0.0:3000`。"仅内网可访问"在**网络层**收口（应用层不负责这个）：
...
> 内网无互联网出口，**不能**让内网服务器直接连阿里云拉数据。请在"还能联网"的环境先把数据导出，再拷进内网。
...
1. 在有网机器上配置 `.env`（指向阿里云 MySQL），运行仓库内置迁移脚本反向导出
...
- **默认口令**：`admin/admin123` 等仅为首次便捷，**务必改密**并关闭 `SEED_DEFAULT_USERS`。
...
4. 若要从 json 切到 mysql：先把 `data/store.json` 用迁移脚本导入 MySQL，再切 `DB_DRIVER=mysql` 重启。
