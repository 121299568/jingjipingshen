...
经济评审管理系统，支持项目资料上传、专家工作量评估、成本核算全流程管理。
...
不想动生产/本地 `data/store.json` 就能跑通全链路？用内置的**内存虚拟数据集**：

```powershell
cd backend
npm run test:pre-launch      # 以 DB_DRIVER=memory 起服务并跑 22 项冒烟断言
```

- 数据**纯内存、不落盘、重启复原**，含 8 个项目 / 2 个事业部 / 3 种状态 / 6 条专家评估，覆盖月度趋势、成本结构、权限隔离。
- 也可用 `DB_DRIVER=memory node server.js` 手动起一个临时测试实例（同样不污染磁盘）。
- 该模式仅供测试，**生产务必用 `json` 或 `mysql`**。
